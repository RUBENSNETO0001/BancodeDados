SELECT aluno_id 
FROM inscricoes 
WHERE nota > (SELECT AVG(nota) FROM inscricoes WHERE curso_id = inscricoes.curso_id);
-- aqui to vendo se os alunos com nota maior do curso serem mostrada
...
SELECT nome  FROM alunos WHERE aluno_id IN (SELECT aluno_id from inscricoes WHERE nota > 80);